...
$(document).ready(function ($) {
  ...
  // function zabutoCalendar() {
  //   if ($(".calendar").length) {
  //     $(".calendar").zabuto_calendar({
  //       ajax: {
  //         url: themeData.urlCalendar,
  //         modal: true,
  //       },

  //       action: function () {
  //         const selectedDate = this.id;
  //         const dateData = getDateData(this.id);
  //         checkDate(selectedDate, dateData);
  //         return dateData;
  //       },

  //       action_nav: function () {
  //         // Задержка отправки запроса на 500 миллисекунд (0.5 секунды)
  //       },
  ...
  // function checkDate(selectedDate, dateData) {
  //   // Ваш существующий код
  //   $("#modal").modal();
  //   // Дополнительные действия, используя dateData, если это необходимо
  //   sessionStorage.setItem("dateData", JSON.stringify(dateData));
  // }

  //  Smooth Scroll

  $('.main-nav a[href^="#"], a[href^="#"].scroll, .scroll-top-arrow').on(
    "click",
    function (e) {
      e.preventDefault();
  ...
  $("#form-daily-motivation button").on("click", function () {
    $("#form-daily-motivation").validate({
      submitHandler: function () {
        $.post(
          "assets/external/daily_motivation.php",
          $("#form-daily-motivation").serialize(),
          function (response) {
