...
<?php
  ...
  foreach ($myrows as $item)
      echo '<option value="'.esc_attr($item->id).'"'.($item->id==$this->item?' selected':'').'>'.esc_html($item->form_name).'</option>';
?>
            </select>
                    </td>
                </tr>
                ...
                <tr valign="top"  id="ppage" style="display:none">
					...
					<td valign="top">
                       ...
                       <h3 style="background:#cccccc; padding:5px;"><?php _e('Using classic WordPress editor or other editors?','appointment-hour-booking'); ?></h3>
                        ...
                        <?php _e('You can also publish the form in a post/page, use the dedicated icon','appointment-hour-booking'); ?> <?php echo '<img hspace="5" src="'.plugins_url('/images/cp_form.gif', __FILE__).'" alt="'.esc_attr(__('Insert '.$this->plugin_name)).'" /></a>';     ?>
...
<div id="metabox_basic_settings" class="postbox" >
  ...
  <div class="inside">
   <?php _e('You can also publish the form in a post/page, use the dedicated icon','appointment-hour-booking'); ?> <?php echo '<img hspace="5" src="'.plugins_url('/images/cp_form.gif', __FILE__).'" alt="'.__('Insert '.esc_attr($this->plugin_name)).'" /></a>';     ?>
