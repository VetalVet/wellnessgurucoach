...
if (isset($_GET['delmark']) && $_GET['delmark'] != '')
{
    ...
    if (isset($_GET['c'.$i]) && $_GET['c'.$i] != '')
        $wpdb->query( $wpdb->prepare('DELETE FROM `'.$wpdb->prefix.$this->table_messages.'` WHERE id=%d', $_GET['c'.$i]) );
	...
	<tbody id="the-list">
     ...
     ?>
	  <tr class='<?php if ($cancelled) { ?>cpappb_cancelled <?php } ?><?php if (($i%2)) { ?>alternate <?php } ?>author-self status-draft format-default iedit' valign="top">
        <th><input type="checkbox" name="c<?php echo $i-($current_page-1)*$records_per_page; ?>" value="<?php echo intval($events[$i]->id); ?>" /></th>
		...
		<td><?php
                ...
                echo $this->filter_allowed_tags(apply_filters( 'cpappb_booking_orders_item', $data, $posted_data ));
		    ?></td>
        <td align="center"><?php echo '<span style="color:#006799;font-weight:bold;">'.(!empty($posted_data["paid"]) && $posted_data["paid"]=='1'?__('Paid','appointment-hour-booking').'</span><br /><em class="cpappsoft">'.esc_html($posted_data["payment_type"]):'').'</em>'; ?></td>
